...
export function registerRollup(program: Command): void {
    program
        ...
        .action(async (opts: { all: boolean; rebuild: boolean; apply: boolean; limit: string }) => {
            ...
            try {
                await withCliReadGeneration(gateDb, async (output, generation) => {
                    ...
                    // for no progress).
                    if (opts.rebuild) {
                        const rebuildStore = new MemoryStore(gateDb);
                        store = rebuildStore;
                        ...
                        if (!opts.apply) {
                            ...
                            output.log(
                                `Preview: ${candidates.length} session(s) would be rebuilt whose rollup predates v${ROLLUP_VERSION}.`,
                            );
                            output.log(
                                `ESTIMATED cost: $${estimateCostUsd(inputTokens, outputTokens).toFixed(4)} (${inputTokens} input / ${outputTokens} output tokens).`,
                            );
                            output.log('Re-run with --rebuild --apply to make API calls.');
                            return;
                        }
                        const { state, healthy } = daemonHealth();
                        if (healthy) {
                            output.error(`Refusing to run --rebuild while the daemon is live (${state}).`);
                            output.error(
                                'Run `elepha pause`, then re-run `elepha rollup --rebuild --apply`, and finish with `elepha resume`.',
                            );
                            process.exitCode = 1;
                            return;
                        }
                    ...
                    if (!providers) {
                        output.error('No synthesis provider configured. Set ANTHROPIC_API_KEY before running rollup.');
                        process.exitCode = 1;
                        return;
                    }
                    const activeStore = store ?? new MemoryStore(gateDb);
                    ...
                    if (opts.rebuild) {
                        output.log(`Rebuilding ${sessions.length} session(s) whose rollup predates v${ROLLUP_VERSION}.`);
                    }
                    ...
                    for (const session of sessions) {
                        ...
                        const parentId = classification.parentNativeId
                            ...
                            : null;
                        const outcome = await service.rollupSession(
                            session,
                            classification.kind,
                            parentId,
                            idle ? 'final' : 'live',
